...
powers_csv = './Prepared_data/Fahrkurve_130_Brand_636.csv'
...
horizon_lengths = [4, 8, 12, 16, 20, 24, 28, 32, 48, 96]
...
hours.each do |hour|
  minutes.each do |minute|
    ...
    timeline_max = (Time.utc(2016, 12, 31, 23, 45, 0) -
    ...
    horizon_lengths.each do |horizon_length|
      ...
      0.upto(number_horizons - 1) do |i|
        ...
        # Calculate the realized process costs, the process utilties and the process ends
        # Declare arrays for the realized process costs, process utilties and the process ends
        process_costs_spot = []
        process_utilities_spot = []
        process_ends = []
        for t in (0..horizon_length - 1) do
          process_costs_spot[t] = 0
          process_utilities_spot[t] = 3000
          # Calculate the processs ends
          process_ends << process_starts[t] +
              process_data.length - 1
          next if process_ends[t] > horizon_end
          # Calculate the realized process costs and the process utilties
          process_data.each_with_index do |power, t_process|
            process_costs_spot[t] += power *
                horizon_closing_prices[t + t_process]
            process_utilities_spot[t] -= power *
                horizon_closing_prices[t + t_process]
          end
        end
          ...
          for t in 0..horizon_length do
            ...
            if t == 0 || n == 0
              ...
              # Process can be started
                # if target output allows to start at least one process
              ...
              pack = process_utilities_spot[t - 1] +
        ...
        # Calculate the realized costs of the schedule
        n = target_output
        t = horizon_length
        while t > 0 && n > 0
          # Process is started
    ...
    CSV.open('./Planning_Horizons_Sc3.csv', 'a+', col_sep: ';') do |row|
      row << ['Start', 'Planning Horizon Length [h]', 'Costs Scenario 3']
